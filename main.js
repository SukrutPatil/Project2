...
app.post("/viewProduct", (req, res) => {
  ...
  if (usertype == "admin") {
    con.query(
      `SELECT * FROM nusta_product_details where admin_email = '${ema}'`,
      function (err, result) {
        if (err) throw err;
        else {
...
app.post("/addProduct", (req, res) => {
  ...
  con.query(
    `insert into nusta_product_details(product_name, sub_text,price,discount,description,product_image,brand,model,model_name,dimen,manufacturer,time_date,admin_email) values ('${req.body.product_name}','${req.body.sub_text}','${req.body.price}','${req.body.discount}','${req.body.pdescription}','${filename}','${req.body.brand}','${req.body.model}','${req.body.model_name}','${req.body.dimen}','${req.body.manufacturer}','${req.body.time_date}','${req.body.emailid}')`,
...
app.post("/coinAdded", (req, res) => {
  con.query(
    `select * from nusta_user_details where email = '${emailid}'`,
    function (err, result) {
  ...
  con.query(
    `UPDATE nusta_user_details SET coins = '${newcoin}'  WHERE email = '${emailid}'`,
    function (err, result, fields) {
      coins = newcoin;
...
const getAllReferrals = async (ref) => {
  return await new Promise((resolve, reject) => {
    con.query(
      `select * from nusta_user_details where r_code = '${ref}'`,
      (err, result) => {
...
app.post("/checkLogin", async (req, res) => {
  ...
  con.query(
    `SELECT * FROM nusta_user_details where email = '${emailid}' and password = '${password}'`,
      ...
      try {
        ...
        if (usertype == "admin") {
          ...
          res.render("afterLogin", {
            allReferrals,
            coins: coins,
            userimage: result[0].photo,
            mailid: emailid,
            THEREFCODE: currentUserRefCode,
            THEREFEARNINGS: refEarnings,
...
app.post("/signupcheck", async (req, res) => {
  ...
  await new Promise((resolve, reject) => {
    con.query(
      `insert into nusta_user_details(email, password,r_code,phone,refercode_use,joindate) values ('${emaill}','${pwd}','${rc}','${phoneno}','${generateReferralCode()}','${getCurrentDate()}')`,
      function (err, result, fields) {
  ...
  if (rc) {
    ...
    let ox = await new Promise((resolve, reject) => {
      con.query(
        `select user_id,coins from nusta_user_details where refercode_use = '${rc}'`,
        (err, result) => {
          if (err) console.log(err);
          console.log(result);
          if (result && result.length == 0) {
    ...
    if (ox)
      await new Promise((resolve, reject) => {
        con.query(
          `UPDATE nusta_user_details SET coins = ${nc} WHERE user_id = '${i}'`,
          (err, result) => {
            if (err) console.log(err);
            console.log("UPDATED");
            resolve(true);
    ...
    if (req.files.file) {
      ...
      file.mv("./public/images/" + filename, function (err) {
        if (err) {
          ...
          if (err) throw err;
          con.query(
            `UPDATE nusta_user_details SET photo = '${filename}'  WHERE email = '${emailid}'`,
            function (err, result, fields) {
              if (err) throw err;
            }
          );
    ...
    if (req.files.idfile) {
      ...
      idfile.mv("./public/images/" + idname, function (err) {
        if (err) {
          ...
          console.log("Connected!");
          con.query(
            `UPDATE nusta_user_details SET idproof = '${idname}'  WHERE email = '${emailid}'`,
            function (err, result, fields) {
              if (err) throw err;
            }
          );
    ...
    if (req.files.addffile) {
      ...
      addffile.mv("./public/images/" + addfname, function (err) {
        if (err) {
          ...
          if (err) throw err;
          con.query(
            `UPDATE nusta_user_details SET addressfront = '${addfname}'  WHERE email = '${emailid}'`,
            function (err, result, fields) {
              if (err) throw err;
            }
          );
    ...
    if (req.files.addbfile) {
      ...
      addbfile.mv("./public/images/" + addbname, function (err) {
        if (err) {
          ...
          if (err) throw err;
          con.query(
            `UPDATE nusta_user_details SET addressback = '${addbname}'  WHERE email = '${emailid}'`,
            function (err, result, fields) {
              if (err) throw err;
            }
          );
      ...
      function (err, result) {
        try {
          res.render("dashboard", {
            coins: result[0].coins,
            mailid: emailid,
            idimage: result[0].idproof,
            addfimage: result[0].addressfront,
            addbimage: result[0].addressback,
            userimage: result[0].photo,
            phone: phone,
            firstname: result[0].firstname,
            lastname: result[0].lastname,
