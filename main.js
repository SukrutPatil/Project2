...
app.post("/checkLogin", (req, res) => {
  ...
  con.query(`SELECT * FROM nusta_user_details where email = '${emailid}' and password = '${password}'`, function (err, result, fields) {
    ...
    try {
      ...
      res.render("afterLogin", {coins:coins,userimage:result[0].photo, mailid: emailid, firstname: result[0].firstname, lastname: result[0].lastname, gender: result[0].gender, dob: result[0].dob, houseno: result[0].houseno, address: result[0].address, village: result[0].village, city: result[0].city, state: result[0].state, pincode: result[0].pincode });
...
app.post("/signupcheck", (req, res) => {
  ...
  con.query(`insert into nusta_user_details(email, password,r_code,phone) values ('${emaill}','${pwd}','${rc}','${phoneno}')`, function (err, result, fields) {
    res.render("authentication-login1");
